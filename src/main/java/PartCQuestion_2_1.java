...
/**
 * Driver for PartC Question 2 with custom partioner as String Custom
 * Partitioner
 */
public class PartCQuestion_2_1 {
    public static void main(String[] args) throws InterruptedException {
        SparkConf sparkConf = new SparkConf().setAppName(APP_NAME + "2");
        JavaSparkContext javaSparkContext = new JavaSparkContext(sparkConf);
        PageRank pageRankRunner = new QTwoRunner_1();
        int exitStatus = 0;
        try {
    ...
    private static Integer extractPartitions(String[] args) {
        Integer partitions;
        if (args.length < 3) {
            partitions = QTwoRunner_1.NUM_PARTITIONS;
        } else {
