...
/**
 * Driver for PartC Question 2 with Custom Partitioner as IntCustomPartitioner
 */
public class PartCQuestion_2_2 {
    public static void main(String[] args) throws InterruptedException {
        SparkConf sparkConf = new SparkConf().setAppName(APP_NAME + "2");
        JavaSparkContext javaSparkContext = new JavaSparkContext(sparkConf);
        PageRank pageRankRunner = new QTwoRunner_2();
        int exitStatus = 0;
        try {
    ...
    private static Integer extractPartitions(String[] args) {
        Integer partitions;
        if (args.length < 3) {
            partitions = QTwoRunner_2.NUM_PARTITIONS;
        } else {
