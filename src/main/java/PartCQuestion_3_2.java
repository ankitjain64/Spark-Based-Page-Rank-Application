...
/**
 * Driver for PartC Question 3 with partitioner as Int Custom Partitioner
 */
public class PartCQuestion_3_2 {
    public static void main(String[] args) throws InterruptedException {
        SparkConf sparkConf = new SparkConf().setAppName(APP_NAME + "3");
        JavaSparkContext javaSparkContext = new JavaSparkContext(sparkConf);
        PageRank pageRankRunner = new QThreeRunner_2();
        int exitStatus = 0;
        try {
            Integer partitions = extractNumberOfPartitions(args);
    ...
    private static Integer extractNumberOfPartitions(String[] args) {
        Integer partitions;
        if (args.length < 3) {
            partitions = QThreeRunner_2.NUM_PARTITIONS;
        } else {
